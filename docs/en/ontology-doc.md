...
| `concept_relations[].roles[]` | a list of texts | K3: `participation` only -- what the entity is to the event, taken from the `from` side's column comment and stripped as a key comment is (发送方编号 → 发送方); when the metadata says nothing it falls back to the column *name*, read as words rather than as an identifier (K4d): the key markers come off, `_` becomes a space (`collection_unit_id` → `collection unit`), a camelCase name is split at its own boundaries, and a CJK name is used as it stands. **The raw identifier is never published** -- a role is a word a business uses, and putting the warehouse's spelling there says the business calls it that. One group can carry several (发送方 and 接收方 are two edges between the same two concepts) |
