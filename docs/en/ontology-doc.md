...
| `batch-NN.md` | this batch's worksheet: the concept table (name and tier, kind, **kind evidence**, **possible duplicates**, tables, **memberships the table already has**, relation and task counts, the first three name candidates, the write-back key), the candidate merge targets (the concepts the corpus already folded, ranked by **match score**), the evidence (table and key-column comments, falling back to **attribute clues**) and this batch's open item groups | the reviewer (an agent or a person) |
...
- **Match score**: the candidate merge targets are ranked by a weighted sum instead --
  **3 per shared name word, 2 per shared key stem, 1 per relation** -- with generic stems
  (`id`, `dt`, the `GENERIC_STEMS` list) counting for nothing. Each row prints the three
  counts it was reached from, and a target that scored 0 is not listed. The name outranks
  the relation: "both sides are called the same thing" is closer to one thing than "some
  task wrote both into one query".
