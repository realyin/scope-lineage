...
| `concepts[].tables[].membership_basis` | `key:<tier>` / `declared_hint` / `reference` / `override` | K1: what makes this table a member -- a candidate key the corpus read (carrying its own tier), a primary-key hint the catalog declared, or a JOIN; `override` is K4b's fourth: a reviewer put it there by hand with `add_tables`, and that member also carries `role_tier: "confirmed"`. K4d: a reviewed add whose role is `reference` publishes **`reference`**, not `override` -- "it merely carries this key" is exactly what `reference` means, and it must not turn round and change what the table itself is; the basis can then no longer say who put it there, so `confirmed_by` / `confirmed_basis` and the rest of the stamp stay on that member row. A `reference` member also lends **no attributes**: it carries the key and is not described by it, so its columns neither become a row of `concepts[].attributes[]` of their own nor join the `sources[]` of a stem the other members carry -- one rule, read the same way for a membership the corpus folded and one a reviewer wrote (see `concepts[].attributes[]`) |
| `concepts[].attributes[]` | `stem` + `type` + `comment` + `sources[]` (`table` / `column`) | K1: what the concept is made of -- its members' columns folded one row per stem, `type` and `comment` from the first source that says anything, `sources[]` naming the table and column behind each. **Only members whose role is not `reference`** (K4d): a reference merely carries the concept's key and is not described by it, so not one of its columns joins the list. The list is re-read off the **current** roles in `tables[]` every time rather than accumulated edit by edit -- a `reference` member a reviewer added with `add_tables` / `new_concepts` still lends nothing, and once a merge lifts a table out of `reference` into a stronger role its columns count from that moment |
...
| `concepts[].tier` | `implied` / `hypothesis` / `confirmed` / **`provisional`** (M1) | K1: how sure the corpus is of the concept. `provisional` is M1's fourth value and is not the same kind of statement as the other three -- it does not say how believable the concept is, it says this is not a concept yet but a table waiting to be merged. N8b: a reviewer who writes a `name` or a `kind` for a provisional concept has taken M1's third way out ("it really is its own thing"), and the tier **leaves `provisional`** there and then -- `implied` when the kind is settled (unanimous votes, or the reviewer's own `confirmed`), `hypothesis` when it is not, and never `confirmed`, which a concept inferred over a corpus is not. **The id does not move**, and its one member is republished as `membership_basis: "override"` / `role_tier: "confirmed"`, without which the table's edges would have nowhere to land |
...
| `concepts[].review_note` | text | N8b: **present only when a reviewed file said `leave`** -- why this round put the concept down. It does nothing to the concept itself; the next `--review-batches` worksheet prints it in that concept's row, so the next reader does not spend half an hour reaching the same "not yet" |
...
carries this key" is not an answer to "what is this table"). N8b: the third way out -- **it really is its own thing** -- now works too: a `name` or a `kind` written for it takes its `tier` off `provisional`, and `provisional_count`, the appendix list, the next `--review-batches` queue and the dictionary's `concept_terms[]` all follow, with the id unchanged. What cannot be answered gets a `leave` and keeps its reason:
...
| `add_tables` | `{"<table>": "<role>"}` | **adds** a table of this corpus to the concept (`roles` can only move a member the corpus already found). The table must appear in `tables[]` and the role is still one of the six; the member carries `membership_basis: "override"` and `role_tier: "confirmed"`, its columns join the concept's `attributes[]` (every role but `reference`, see below), and that table's **provisional concept dissolves** (M1), reported in `concept_overrides_applied.dissolved[]`. **The `reference` role is the exception** (K4d): it publishes `membership_basis: "reference"` with the stamp kept on the member row, and what the table itself is does not move -- "it merely carries this key" must not turn round and lend an identity, **and its columns stay out of `attributes[]`**. One table may be added to several concepts (a detail table carrying two keys), but **identity is single**: a table its own key already placed on a concept only *carries* the key of any concept it is added to, and K3 still folds its edges from the concept that identified it. This runs before the concept relations are folded, so the JOINs that start at the table land on the concept the reviewer named. **N8b: naming a table that is only a `reference` member of this concept is not `already_a_member` but an upgrade** -- `reference` says no more than "the corpus watched a JOIN travel on this key", and a reviewed role answers the question it left open. That membership moves in place to the reviewed role and basis at `role_tier: "confirmed"`, the stamp lands on the member row, its `key_columns` stay (it really does carry that key), its columns **start counting** towards `attributes[]` (N9a), and the table's provisional concept dissolves with it; reported in `roles_upgraded`, not `tables_added`. A table already holding any other role -- or asked for as a `reference` again -- is still refused with `already_a_member` |
| `merge_into` | another concept id | folds this concept into that one: its tables, attributes and key stem all travel, and its id is kept in the survivor's `merged_from[]`. K4d: a table both sides hold keeps the **stronger** of the two roles (`primary` > `snapshot` > `detail` > `summary` > `intermediate` > `reference`) rather than always the survivor's row -- what the folded concept read off that table should not be lost to the merge. The attributes are re-read off the roles the merge settled on: a table lifted out of `reference` counts from then on, and a table that is a `reference` on both sides still lends nothing. When each side has its own, *different*, `primary` copy both rows stay (which one is *the* copy is the business's answer, not this layer's) and `warnings[] = {key, warning: "merge_kept_two_primaries: <t1>, <t2>"}` is published: a merge has a direction, and the side with fewer primaries goes into the other. N1b: none of that applies when the folded side is a **provisional** concept -- it is one table, and by M1's construction that table is its `primary` -- so its member is **re-roled** as it joins: `snapshot` / `summary` / `intermediate` / `detail` by its own table suffix and producing grain when the survivor already has a primary, `primary` when it has none, published as `membership_basis: "override"` at `role_tier: "confirmed"`. Such a merge no longer raises `merge_kept_two_primaries` |
| `merge_role` | a role string, or `{"<table>": "<role>"}` | N8b: **only read beside a `merge_into`** -- which copy of the surviving concept the folded members are. A string is that role for every member the fold carries; a mapping says it per table, and a member the mapping does not name lands by the old rules (a provisional member re-roled, every other one unchanged). It outranks both of those and publishes `membership_basis: "override"` at `role_tier: "confirmed"`. A table both sides hold still keeps the **stronger** of the two roles (K4d). A role outside the six is reported as `unknown_role: <value>` and dropped while **the merge still happens** -- refusing the whole fold over a misspelt role would throw away the answer the reviewer did give |
| `leave` | one sentence | N8b: **"not this round, because…"**. It does nothing to the concept -- the reading is unchanged, `concepts` does not count it, and the concept stays in the provisional pile -- and leaves only the reason: `review_note` on the concept (the next worksheet prints it in that row's 上轮留待 column) and one `concept_overrides_applied.left[] = {id, reason}`. Deleting an entry nobody could answer takes the reason with it, and the next round starts over |
| `new_concepts[]` | `{id, name, kind, tables, key_columns?, …}` | K4c: **creates** a concept the corpus never seeded. The `id` must be unused and slug-shaped, `concept:<lowercase stem>` (otherwise `already_a_concept: <id>` / `invalid_concept_id: <id>`); the keys of `tables` are tables of this corpus and the values their roles, and a table another concept already holds **by identity** may only take the `reference` role (otherwise `already_a_member: <table>`). The created concept carries `tier` / `name_tier` / `kind_tier` all `confirmed` and `origin: "override"`, its `identity.stem` is the id's stem, its `identity.columns_seen` is `key_columns` or the key columns its tables share, its attributes come from the members whose role is not `reference` (K4d reads the same here: a reference carries the key and lends no attributes), and those members' **provisional concepts dissolve** (M1), reported in `dissolved[]`; a provisional concept does not count as "another concept", so a member never reports `already_a_member` because of one. K4d: the stems those key columns themselves reduce to (the non-generic ones that are not already the id's stem) join `identity.merged_stems[]`, which is the stem index K3 reads when it folds an edge -- that is how an edge written on `ad_slot_code` reaches `concept:slot`. It lands **before** the merges and the splits, and before the concept relations are folded |
...
| `batch-NN.md` | this batch's worksheet: the concept table (name and tier, kind, **kind evidence**, **kind tier**, **possible duplicates**, tables, **memberships the table already has**, relation and task counts, the first three name candidates, **what a previous round left open**, the write-back key), the candidate merge targets (the concepts the corpus already folded, ranked by **match score**), the evidence (table and key-column comments, falling back to **attribute clues**) and this batch's open item groups | the reviewer (an agent or a person) |
...
Every column of the worksheet is part of one thing: deciding without leaving the batch (N1b).

- **Memberships the table already has**: which folded concepts **already** claim this table,
  with the role and the basis of each (read straight off `tables[].concepts[]`; the table's
  own provisional concept is not one). A `merge_into` onto one of them collides with that
  membership -- the same thing `add_tables` reports as `already_a_member` -- and the
  **stronger** of the two roles is what survives.
- **Kind evidence**: `kind_evidence[]` compacted into one cell, grouped by vote --
  `event: key_event_column×2 / entity: word_hint×1`. Two signals disagreeing is visible
  without going back to `ontology.json`.
- **Kind tier** (N8b): the `kind_tier` in the cell next to it. The evidence says which signals
  voted; the tier says whether **the rules allow self-answering** -- `implied` means answer it,
  `hypothesis` means it is worth one of the eight human questions. Deriving that from the votes
  meant doing it in your head, or going back to `ontology.json`.
- **Possible duplicates**: `possible_duplicate_of[]` -- another concept claims this name too.
- **Match score**: the candidate merge targets are ranked by a weighted sum instead --
  **3 per shared name word, 2 per shared key stem, 2 per table-name root (N8b), 1 per
  relation** -- with generic stems (`id`, `dt`, the `GENERIC_STEMS` list) counting for
  nothing. Each row prints the four counts it was reached from, and a target that scored 0
  is not listed. The name outranks the relation: "both sides are called the same thing" is
  closer to one thing than "some task wrote both into one query".
- **Table-name root** (N8b): the candidate's `identity.stem` appears as a whole word in one of
  this batch's **table names** (split on `.` and `_`; `ods.slot_log_di` matches
  `concept:slot`, while a prefix does not -- `omegax_rows` is no table of `omega`). The
  warehouse's own naming convention is evidence too, and it is the only kind a table **nothing
  keyed and nothing joined** still carries -- such a candidate used to score nothing at all,
  and the whole section came out empty.
- **What a previous round left open** (N8b): the concept's `review_note`, the sentence written
  with `leave` last time.
- **Attribute clues**: when a table has neither a comment nor a candidate key -- M1 at its
  barest -- the evidence cell prints the **first 8 of its columns that carry a comment**
  instead, which is all the corpus has left to say about it.
