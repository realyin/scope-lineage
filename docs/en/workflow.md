...
| `semantic packet` / `validate` / `confirm` / `render` | a `lineage.json` tree, the task JSON directory, `--schema`; later the written `table-semantics/1` documents, a `semantic-confirmations/1` file, and optionally the validation report and a built `ontology.json` | `<db.table>/packet.md` + `packet.json`; a text or `--json` validation report; the confirmed documents; one `<db.table>.md` page per table and `index.md` | a model writes each table's document from its packet; the owner answers its questions | [Table semantics](table-semantics.md) |
