...
| expressions nest so deep the task ends partial | `parse --expansion-limit N` | how many upstream expressions may be inlined into one `expanded_expression` (default 2000); a task that hits the limit ends with an `expression_expansion_bounded` gap |
