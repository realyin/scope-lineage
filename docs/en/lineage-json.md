...
Masking happens **at collection time**, on the comment text itself, so every comment key above and the inline copy inside a rendered expression (such as `logic_blocks[].raw_expression`) all carry the same masked text; `task_meta.description` goes through it too, being free text a person wrote.

**A comment loaded from metadata follows the same rule (E1)**: the column comments and table-level text read from `--schema` / `--target-ddl-metadata` were written by a person and are published in the same artifacts, so they are masked once where `related_metadata` is assembled — `related_metadata.*.column_details[].comment`, `declared_columns[].comment`, `table_metadata.table_desc` / `table_name_cn`, and the copy of the same text under `scopes[].inputs[]` (`used_columns[].comment`, `source_metadata`). `--no-redact-comments` turns both kinds off together. The other `table_metadata` keys (domain, project, owner, layer, source file) are identifiers and classifications the export produced and are not rewritten — changing a digit there corrupts a fact rather than protecting anybody.

**A confirmed comment written back is masked too**: the `columns[].comment` and `tables[].table_name_cn` / `table_desc` of a `--metadata-patch` file go through the same masking as the patch is read. A patch is applied after parsing, so masking it anywhere later would leave the one comment a person typed by hand as the one comment published verbatim — and a reviewed file is exactly where somebody writes a colleague's contact details down. `--no-redact-comments` turns this off as well.

The SQL expression itself is never rewritten: `WHERE id_no = '110101199003078219'` is data the statement operates on, and masking it would change what the SQL says.
