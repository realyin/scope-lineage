...
| `join_keys_not_split` | A JOIN's equality keys could not be split from its ON condition (self-joins whose qualifiers cannot be sided, `ON TRUE`, and the like); the verbatim condition is kept. | Read the join's `condition_expression` / mapping.md §6 "连接条件（未拆分）" lines instead of `join_key_pairs`; do not treat the empty key list as "no join keys". |
