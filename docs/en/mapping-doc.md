...
| --- | --- | --- | --- |
| 1. 概览 (Overview) | overview | Task, target table, statement kind, partitioning, parse status, target-binding summary | Top-level fields, `target_field_binding`; a statement with no binding to make is written as 「不适用（…）」 from its `reason`, a missing-metadata case gets its reason in Chinese from `target_binding_absent_reason`, and only `target_table_not_found` (the one case that risks landing in the wrong column) is marked ⚠ |
