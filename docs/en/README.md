...
14. [`ontology.json` / `ontology.md` corpus-level ontology candidate](ontology-doc.md): use `scope-lineage ontology` to turn a whole corpus, on top of the cards and the dictionary, into a tiered ontology candidate — the business concepts and the relations between them, the tables that represent them and the JOIN evidence beneath, identity keys, constraints, and cross-task contradictions; the index opens on the concept overview and links each concept to its own `concepts/<file>.md` (everything table-level is in `appendix.md`), every table card gains five sections (identity / relations / constraints / synonyms / open items), and human answers come back through `--overrides` as a fifth tier, `confirmed`; the business naming and the class hierarchy are left to a person or an agent.
15. [Ontology catalog (`catalog-yaml/1`) and `ontology-json/3`](ontology-catalog.md): the concept-first source of truth a person maintains — domains, identifiers, entities, events, roles, states, relations, constraints, terms, and the mapping of tables and columns onto them; `scope-lineage catalog validate` checks it and `scope-lineage catalog build` normalises it into `ontology-json/3`. The `ontology` command above stays for one more release.
16. [Table semantics (`table-semantics/1`)](table-semantics.md): what one target table means, written for a business reader by a model and held to the facts by machine — `scope-lineage semantic packet` gathers each table's metadata, producing tasks with their SQL, input tables and lineage facts into a material packet, `scope-lineage semantic validate` checks a written document against its schema and the packet (thirteen cross checks, a per-item rewrite list), `scope-lineage semantic confirm` writes a person's answers back, and `scope-lineage semantic render` renders one page per table and an index, linked to the catalog's concept pages.
17. [AI agent skill](agent-skill.md): let Claude Code, Codex, and other AI coding agents use lineage parsing, field transformation chains, and mapping documents directly.
