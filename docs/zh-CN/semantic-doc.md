...
# 接上语料三层：表卡、值词典、概念层（--ontology 只接受 ontology-json/2）
scope-lineage describe --lineage /path/to/corpus --out /path/to/docs \
...
- 选项变了就整份作废、全部重算：`--overrides` 文件的**内容**、`--format`、读回来的
  `glossary.json` / `tables.json` / `ontology.json`、以及工具版本，任何一项对不上，索引就当没有。
...
| 块 | 关键子键 | 取值词表（以实现为准） |
| --- | --- | --- |
| `task` | `target_table_comment`、`target_table_domain` / `target_table_project` / `target_table_owner`（目标表的业务归属，取自元数据的表级事实，`null` 表示元数据没说）、`partition`、`instance_dates[]`、`driving_tables[]`、`structural_summary`、`target_metadata_source`、`header_comments[]`、`meta` | `partition.mode`：`static` / `dynamic`；`target_metadata_source`：`schema` / `target_ddl` / `null`（两侧都没描述该表）；`header_comments[]` 是契约 `statement_comments` 的原样副本（语句头部注释，作者原话，恒存在、可为空；脚本头部注释块写在 `SET` 等未建模语句之上时，契约已把它并入第一条写入语句，因此也在这里出现）；`meta` 是契约 2.0 顶层 `task_meta` 的原样副本（含 B4 的 `upstream_tasks` / `downstream_tasks` 两个任务名数组：那是**调度登记**，与语料证明的 `downstream_consumers` 不是一回事），`.sql` 输入或无任务元信息时为 `null`——`null` 表示没有输入提供，不表示这个任务没有负责人或调度。两者都是**引用**而不是本视图的断言：注释与元数据里的列注释可能互相矛盾，产物不替作者裁决；`instance_dates[]`（WI-2.9）是本实例的等值日期过滤钉住的**日期形字面量**去引号去重升序后的集合（`['20260814']`），`[]` 表示没有任何过滤钉住一天——由 `${…}` 变量替换，或根本没有日期过滤，**不表示读取全部日期**；判定只看常量的写法（`yyyyMMdd` / `yyyy-MM-dd` / `yyyy/MM/dd` 前缀），与 `hardcoded_date_literal` 读的是同一批比较，两者不会指向不同的天；`driving_tables[]`（B2）是**每一条输出行的来源物理表**，每条为 `{table, via_scopes[], lateral_view_scopes[]}`：从 ROOT 出发只走 FROM 项（JOIN 右侧永不算 FROM 项，RIGHT JOIN 换边、FULL JOIN 两侧都算、UNION 取全部分支），遇到 LATERAL VIEW **不停**（展开放大的是主表的行，不是换了一张主表），落到物理表为止；`via_scopes[]` 与 `grain.via_scopes` 同向（自 ROOT 向下，不含 ROOT），`lateral_view_scopes[]` 是这条路径上带 LATERAL VIEW 的 scope。**每种形态都有驱动路径**（P3）：聚合 / 去重的 ROOT 从聚合 / 去重 scope 的最左 FROM 项照样往下走，MERGE 走 USING 那一侧——行**数**由键集决定，行的**来源**仍是某张表，读者问的「这张表的行从哪来」问的正是后者（旧版这三种形态恒为 `[]`，真实语料里大多数语句因此答不出来）；它不改变任何角色取值，聚合来源表的 `role_in_task` 仍是 `aggregate_source`，同一条路径的按表标记是 `inputs[].driving`（见下一行）；`structural_summary` 因此按形态开头：投影形态写「行来源 <表>（经 <scope> → <scope> 两次 LATERAL VIEW 展开）；补充 <ROOT 直接读取的其余表>」（句子按数据流方向自表向上写，与 `via_scopes[]` 相反），聚合形态写「按 <粒度键> 汇总，行来自 <表>（经 …）；补充 …」，空分组集（`grain.basis` 为 `single_row`）写「全表汇总，行来自 …」，去重形态写「按 <键> 去重，行来自 …」，键多于 3 个时只列前 3 个并写「等 N 列」（整份键列表在 `output_shape.grain.keys`）；判不出驱动路径时退回旧的「ROOT 直接读取 …」写法 |
| `inputs[]` | `comment`（表注释，按 `table_name_cn` → `table_desc` → `comment` → `table_comment` 取第一个有值的）、`domain` / `project` / `owner` / `layer`（表级元数据事实，恒存在、缺则 `null`）、`role_in_task`（最具体的一个）、`driving`（P3，仅在为 `true` 时出现）、`roles`（全部）、`used_columns[].usages`、`metadata_complete`、`table_column_count` | 角色（R7）：`driving`、`merge_source`、`aggregate_source`、`dedup_source`、`union_branch`、`filter_partner`、`enrich`、`rowset_only`；`driving` 以 `task.driving_tables[]` 的驱动路径为准（B2）：路径判得出时，路径之外的表拿不到 `driving`，RIGHT JOIN 的左侧因此退回 `enrich`；`filter_partner`（B2）是驱动路径上 INNER / SEMI / ANTI JOIN 的另一侧——它既补列又会把连不上的主表行**丢掉**，写成 `enrich` 等于否认后一件事；`driving` 除 ROOT 直读的主表外，还包含**下层 scope 的 FROM 项**：某个非 ROOT scope 的 FROM 项（按 `_scope_from_item` 规则，只做 JOIN 右侧的输入不算）所属物理表，只要该 scope 在 ROOT 的驱动路径上（UNION 取全部分支），或它的输出列正是 ROOT 的 GROUP BY 键，就同样是 `driving`——GROUP BY 键全来自某张表却因 ROOT 不直读而被标成 `enrich`，会把读者引到查找表上；MERGE 语句不适用（R2 判不出形态，其来源仍是 `merge_source`）；`driving`（P3）是**另一件事**：它不是角色，而是「这张表在 `task.driving_tables[]` 的驱动路径上」这一条事实的按表标记，每种形态都给（聚合 / 去重 / MERGE 也给），**仅在为 `true` 时出现**——缺席表示这张表不在路径上，不表示本视图判过它；要找主表读它或 `task.driving_tables[]`，不要改读 `role_in_task`：一张聚合来源表同时是 `role_in_task: aggregate_source` 与 `driving: true`，两件事都为真；用途：`filter`、`partition_filter`、`join_key`、`group_by`、`window_partition`、`window_order`、`output`；`used_columns[].term_meaning`（WI-2.12）是语料字典里**已人工确认**的该列名含义 `{text, status}`，与该列自己的 `comment` 并排而不替换它，没有确认过就整键缺席 |
| `output_shape` | `shape`、`grain`、`candidate_keys`、`unexposed_keys`、`key_evidence`、`key_confidence`、`partition_columns`、`fan_out_risks[]` | 形态（R2）：`aggregated`、`deduplicated`、`union_merge`、`enriched_projection`、`filtered_projection`、`unknown`；`grain.basis`（R3）：`group_by`、`distinct`、`window_partition`、`driving_table_rows`、`single_row`、`unknown`——`single_row`（B10）是**聚合但分组集为空**（`SELECT COUNT(1) … FROM t`，整条语句没有 GROUP BY 子句）：整张输出一行，`grain.keys` 为空而 `key_confidence` 恒为 `proven`（空键集平凡唯一），`candidate_keys` 为空；UNION ALL 若干个这样的聚合**不是**单行，粒度链路上的 UNION 阻断照旧把它答成 `unknown`；`grain.via_scopes`：从 ROOT 递归下探时**穿透过**的 scope_id 列表（不含 ROOT、不含终止的那一层），空列表表示 ROOT 直读；`grain.candidate`（B3）只在 `basis = unknown` 且驱动路径上确有 LATERAL VIEW 展开、而**展开之前那一层的粒度本身判得出**时出现，形如 `{keys[], row_source, basis: "candidate", confidence: "hypothesis", reason, evidence[]}`：`keys[]` 是展开前的粒度键加上各次展开产生的列，`row_source` 只在展开前是 `driving_table_rows`（那一层没有键可列）时给出那张表、否则为 `null`，`evidence[]` 是带 LATERAL VIEW 的 scope。`grain.basis` **仍然是 `unknown`**，`candidate_keys` / `key_confidence` 一概不受它影响——它是结构提示的行形状，不是证明；驱动路径上有多张主表（UNION、FULL JOIN）时不给候选，行数是各分支之和而不是乘积；`grain.keys[]` 是**逻辑键**对象（见下表），`candidate_keys[]` 是**目标表列名**；`key_confidence`：`proven`（GROUP BY / DISTINCT / 窗口 `= 1` 的键，**分组所在 scope 下游**的每个 JOIN 都 `safe`，且每个键都直投到目标列——B12：分组之前（含分组所在 scope 自己）的 JOIN 放大的是被聚合的行，它把数值做大而不会给输出多加一行，分组集在输出里照样唯一，因此不压低键置信；「下游」= ROOT 加上 `grain.via_scopes` 里排在分组 scope 之前的那些 scope，被这样忽略掉的非 `safe` 风险在 `key_evidence[]` 里留一句「`<scope>` 的关联放大发生在分组之前，不影响输出键唯一性」，风险本身照发；`driving_table_rows` / `unknown` 没有这层保护——它们没有任何操作把行变唯一，链路上任何一处放大照旧使它 `none`）/ `proven_unexposed`（键本身成立，但至少一个键没写进目标表——**目标表列无法唯一标识一行**，治理发现）/ `candidate`（主表行粒度，连接键候选且都已输出）/ `none`（此时 `candidate_keys` 为空）；`fan_out_risks[]`：粒度链路上**每个** scope 的 JOIN，带 `scope_id`——MERGE 同样走这条链路（自 ROOT 经 USING 来源往下），只是不发布它算出的粒度（MERGE 写入的行不是来源的行），因此会重复来源行的 JOIN 与 INSERT 一样以 `path = grain` 列出；`status`：`safe` / `risk` / `unknown`——判定发生在**右侧 scope 的逻辑层**：右侧 GROUP BY 项（或排名窗口的 PARTITION BY 项）在该 scope 的输出列名（未投影时用其表达式原文）被 `join_key_pairs[].right.column` 的集合覆盖才是 `safe`，否则 `risk`；**判定前先扣掉被等值钉死的键**（B9）：右侧 scope 自己的 WHERE（或它在驱动路径上原样透传该列的上游 scope 的 WHERE）的**顶层 AND 合取项**里出现 `col = <标量字面量 | ${…}>` 时，这一列在该 scope 内不可能变化，因此不计入唯一键集——`<>`、`IN (a, b)`、`BETWEEN`、`LIKE` 与「和另一列比较」都不算，嵌在 OR 里的等值也不算（契约只按 AND 拆合取项，一个 OR 整体不是等值式），键本身写成 CASE 等表达式时也不算（它只是**读**了那一列）；被扣掉的列发在 `fan_out_risks[].pinned_keys[]`，每条为 `{scope_id, column, value}`，reason 写成「右侧按 … GROUP BY，键集被连接键覆盖（dt 被等值过滤钉死为 '20260815'，不计入键集）」；扣完键集为空时右侧至多一行，同样 `safe`；键的物理穿透列只作为 reason 末尾的附注（`（键的物理来源 …）`），**不参与判定**——一个 CASE 键穿到 4 列、另一个穿到其中 2 列时，物理集合会把「右侧按 3 个键唯一」误读成「被 2 个连接键覆盖」；`path`：`grain`（粒度链路，影响输出行数）/ `argument`（某个指标的**参数路径**上的 JOIN——不改变输出行数，但会重复指标读到的行，把数值放大）/ `anchor`（指标锚定到自己的聚合 scope 后，该 scope **输入子树**里既不在驱动路径也不在参数路径上的 JOIN——它同样会放大被聚合的行；这三条路径都不沾的旁路 JOIN 仍然不列）；**只有 `path = grain` 的风险参与 `candidate_keys` / `key_confidence` 的判定**：参数路径与锚定子树上的放大改的是指标取值而不是行身份，让它压低键置信等于用另一个问题撤销这条语句能给出的最强结论，风险本身照发；`basis` 只在 `describe --tables` 用表卡改判过这条风险时出现，取 `table_card`（见 [tables-doc.md](tables-doc.md)「表卡参与 fan_out 判定」），此时 `candidate_keys` / `key_confidence` 也按新的风险集合一并算出 |
| `output_shape.grain.keys[]` / `unexposed_keys[]` | `scope_id`、`name`、`expression`、`physical_sources[]` | 一个 GROUP BY 项 / 窗口 `PARTITION BY` 项 / DISTINCT 输出列 = **一个**逻辑键，无论它的表达式穿透到几个物理列：`scope_id` 是产生该键的 scope，`name` 是该 scope 的输出列名（GROUP BY 项不是投影列时为 `null`），`expression` 是原表达式，`physical_sources[]` 是穿透到的 `{table, column}` 列表；`pinned`（B9）只在这个键被它自己的 scope 等值过滤钉死为一个值时出现，形如 `{"value": "'20260815'"}`——键**留在** `grain.keys` 里（读者要看见这一行覆盖的是哪一天），但不进 `candidate_keys`、也不因为没写进目标表而算 `proven_unexposed`：一个不可能变化的列标识不了任何一行。`unexposed_keys[]` 是其余键里没有直投到任何目标列的那些 |
...
### 语料概念层：`describe --ontology`

传了一份 [`ontology.json`](ontology-doc.md)（**只接受 `ontology-json/2`**，旧版本直接报错退出并
写明两边的 `doc_format`——`/1` 的 `relations[]` 指的是表级证据，按 `/2` 读会把它当成概念关系）
之后，profile 再多五处，**都按归一表名匹配**，且每一处都只是**复述**本体已经发布的结论，本视图
不新增任何概念判断：

- `task.concepts[]`：本语句读或写的每张表所属的概念，每条为 `{concept, name, kind, role,
  membership_basis, table, direction}`，`direction` 取 `read` / `write`；**按概念去重**，
  同一个概念既被读又被写时只发一条、取**写入**那条（「本任务产出客户的一份表示」是更强的事实，
  读的那一侧在对应 `inputs[]` 上照样看得见），位置仍留在它第一次出现的地方。
- `task.output_concept`：目标表代表的概念与它在其中的角色，`{concept, name, kind, role}`；
  本体不建模这张表时为 `null`。
- `inputs[].concept`：这张输入表的**身份概念**（同样四个键）——它**是什么**，而不是它顺带引用
  了什么：`membership_basis` 为 `reference` 的成员永远不会被当成身份。本体不建模这张表时为 `null`。
- `fields[].concept_attribute`：该目标列的物理来源列（按 `sources[]` 顺序取第一条能被概念层
  安放的）在概念层里的位置，`{concept, name, attribute}`；这一列正是该概念的身份键时**另加**
  `is_concept_key: true`（先问键再问属性）。`count(1)`、常量等没有物理来源的列两个键都不出现。
- `output_shape.grain.concept_text`：把粒度写成「一行 = 一个客户 × 日期」。三条规矩管住它
  「只是在复述粒度」这件事：概念层至少安放了**一半**粒度键才发这个键——四个键只认出一个，
  那不是粒度的另一种说法，那是四分之一个粒度；每个业务词**只写一次**，最多写 6 个
  （`CONCEPT_GRAIN_TERM_LIMIT`）——数仓把三张表按同一个 id 建键时，原来会写出
  「… × 投放单元id × 投放单元id × 投放单元id」，一件事说三遍还把整行撑长；一个概念
  **连同它自己的 id 列**一起算一个词（某个键是 `concept:unit` 的身份、另一个键是别的表上
  同一个 `unit` stem 时，「一个投放单元 × 投放单元id」是同一件事说了两遍，只有前者是概念）。
  **只要没把每个粒度键都写出来**——安放不了的、被折掉的、被上限截掉的——句末一律接
  「等 N 列」，N 是粒度键的真实个数：折叠省掉的正是这个数，它是唯一不能省的东西，
  完整键表在 `grain.keys[]` 里。属性用它自己的注释命名，没有注释才退回折叠后的 stem，
  绝不另造一个词。

**暂定概念（M1）在上面四处出现时一律带 `provisional: true`**，只在为真时出现：一张还没人说它
是什么的表被发布成概念，是给评审轮的提问，不是对业务的断言，读者必须能一眼分辨。

`semantic.md` 相应多三处：第 1 节多一行「- 涉及概念：客户（实体，写入：汇总）、消息发送（事件，
汇总读取）」（概念层覆盖不到本语句的表时写明「本语料的概念层未覆盖本语句涉及的表」，与"没传
`--ontology`"整行不出现区分开）；第 2 节的粒度行在 `concept_text` 存在时改用它（`依据…`、
`basis=…`、物理来源照旧）；第 5 节「完整字段清单」在**至少有一列被安放**时多一列「所属概念」
（写 `客户·cust_name`，该列是身份键时写 `客户·键`，其余写 `—`，暂定概念后缀 `（暂定）`）。

没有传 `--ontology` 时这五个键**根本不出现**，`semantic.json` 与 `semantic.md` 与概念层上线
之前逐字节一致。
...
| --- | --- | --- | --- |
| 1. 任务概览 | overview | 目标表与表注释、语句类型与分区、目标表元数据来源、「本实例取数日」一行（`task.instance_dates` 非空时才出现，措辞注明日期随实例替换，多天时附「（相差 N 天）」）、结构摘要一句、「涉及概念」一行（仅 `describe --ontology`）、任务元信息一行（项目 / 负责人 / 调度周期 / 调度表达式 / 期望日期 / 任务描述，末尾按 B4 追加「上游任务 N 个」「下游任务 N 个」——只写个数不写名字，名字在 `task.meta` 里；缺哪项就不写哪项，全缺则整行不出现）、SQL 头部注释引用块（`> ` 逐行原文，标题「SQL 头部注释（原文，作者说法，非 SQL 事实）」，无注释时整块不出现）、输入表表格（注释、角色、使用列/全宽、元数据完整性、读取 scope） | `task`、`inputs` |
| 2. 输出表形态与粒度 | shape | 形态、粒度的逻辑键与依据（含穿透路径、物理来源）、键/候选键（**目标表列名**，按 `key_confidence` 措辞）、键那行下面每条 B12 说明一行「- 说明：`<scope>` 的关联放大发生在分组之前，不影响输出键唯一性」（取自 `output_shape.key_evidence[]`，没有就不出这行；`key_evidence[]` 里其它的注记不走这行）、分区列、粒度链路上逐 JOIN 的行数放大风险；`path` 为 `argument` 的 JOIN 另起一行小标题「影响指标取值的关联：」单列——它不改变行数，改变的是指标数值；被钉死的粒度键在粒度行里写成 `ROOT.dt（钉死为 '20260815'）`，`grain.basis = single_row` 时粒度行写「一行 = 全表汇总」、键行写「输出只有一行，无需键即可唯一标识」；`grain.concept_text` 存在时（仅 `describe --ontology`）粒度行的键那半句改写成「一行 = 一个客户 × 日期」，`依据…`、`basis=…` 与物理来源照旧；`grain.candidate` 存在时粒度行在「未能判定」之后接「；候选：一行 = `<表>` 的一行 × `<展开列>`（<原因>）[推断]」——`[推断]` 是给画像写作方的记号，照抄时把这个记号一起抄走 | `output_shape` |
...
| 5. 字段语义 | fields（子开关 fields_table） | **先**出"完整字段清单"紧凑表（`#` / 字段 / 一句话语义 / 结构角色 / 口径 / 追溯，另按需插入「所属概念」列，见下，"口径"是聚合调用与时间范围的一句压缩，非指标字段写 `—`），**再**逐字段一个 `###` 小节：第一行 `- 语义：<summary>`（无标签），指标字段紧接一张固定七行的"口径"小卡（统计对象 / 时间范围 / 纳入条件 / 聚合 / 单位·类型 / 空值 / 更新频率；槽位为空时写明"未在聚合路径上发现日期过滤"这类原因，而不是省掉该行；来自参数路径的日期过滤在"时间范围"行末标"（参数来源侧）"，"空值"行在参数可空时写成"参数来自 LEFT JOIN 右侧（<scope>），关联不上时为空"；"更新频率"行在 `task.meta` 给出调度时写成"每日（cron …）"、否则写"未知（任务元信息未提供）"），随后 `- 注释：…（SQL注释）` 一行（该字段推导链上的作者原话，无则整行不出现）、目标注释、类型、来源、结构角色、逐步加工、最终表达式、追溯状态；有取值观察时在注释行后加一行 `- 取值：`（已确认写含义、候选写 `? `、都没有写「待确认」，整列封闭时行尾追加封闭说明）；WI-2.12：目标注释为空而字典有已确认术语时，`- 目标注释：` 之后多一行 `- 术语：<含义>（人工确认）`，「完整字段清单」也相应多一列「术语」（有术语的写 `<含义> ✓`，其余写 `—`），无术语时该列不出现；N5：`describe --ontology` 把至少一列安放进概念层时，「完整字段清单」在「一句话语义」之后多一列「所属概念」（写 `客户·cust_name`，身份键写 `客户·键`，其余写 `—`，暂定概念后缀 `（暂定）`），一列都没安放到时该列不出现 | `fields` |
