...
| 2. 输出表形态与粒度 | shape | 形态、粒度的逻辑键与依据（含穿透路径、物理来源）、键/候选键（**目标表列名**，按 `key_confidence` 措辞）、分区列、粒度链路上逐 JOIN 的行数放大风险；`path` 为 `argument` 的 JOIN 另起一行小标题「影响指标取值的关联：」单列——它不改变行数，改变的是指标数值；被钉死的粒度键在粒度行里写成 `ROOT.dt（钉死为 '20260815'）`，`grain.basis = single_row` 时粒度行写「一行 = 全表汇总」、键行写「输出只有一行，无需键即可唯一标识」 | `output_shape` |
