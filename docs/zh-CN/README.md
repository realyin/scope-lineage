...
16. [表语义（`table-semantics/1`）](table-semantics.md)：一张目标表对业务读者意味着什么，由模型写、由机器对照事实校验——`scope-lineage semantic packet` 把每张表的元数据、生产任务及其 SQL、输入表和血缘事实收成材料包，`scope-lineage semantic validate` 用 Schema 与材料包检查写好的文档（十三项交叉检查，逐条给出重写清单），`scope-lineage semantic confirm` 把人的回答写回，`scope-lineage semantic render` 渲染每表一页和一页索引，并与本体目录的概念页互链。
17. [AI agent 技能](agent-skill.md)：让 Claude Code、Codex 等 AI 编码 agent 直接用上血缘解析、字段加工链和 mapping 文档能力。
