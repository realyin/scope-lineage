...
| --- | --- | --- | --- |
| 1. 概览 | overview | 任务、目标表、语句类型、分区、解析状态、目标绑定摘要 | 顶层字段、`target_field_binding`；本来就没有绑定可做时按它的 `reason` 写成「不适用（…）」，缺元数据时按 `target_binding_absent_reason` 给出中文原因，仅 `target_table_not_found`（唯一有落错列风险的情形）标 ⚠ |
...
每个字段一个 `###` 小节，标题携带完整身份（按下列优先级分派：目录写入 >
未绑定 > merge 分支 > 普通）：
...
- 未绑定目标列（链的 `final_output_fields` 为空——输出名不是可靠的目标列名且
  无目标元数据完成绑定）：生成占位名（契约键 `name_is_generated: true`）为
  `### 字段 <字段>（匿名投影，未绑定目标列）`，其余（如纯数字别名）为
  `### 字段 <字段>（未绑定目标列）`。两者都不拼 `<目标表>.<字段>`——
  目标表并未声明过这个列名，拼出来就是伪造的物理字段 id。
