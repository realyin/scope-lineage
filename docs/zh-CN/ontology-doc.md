...
| `concepts[].tables[].membership_basis` | `key:<层级>` / `declared_hint` / `reference` / `override` | K1：这张表凭什么算这个概念的成员——读到的候选键（带它自己的层级）、元数据声明的主键线索，还是一条 JOIN；`override` 是 K4b 的第四种：评审用 `add_tables` 亲手放进来的，那一条成员还带 `role_tier: "confirmed"`。K4d：评审把角色写成 `reference` 的那一条**发布成 `reference`**，不是 `override`——「只是带着这个键」正是 `reference` 的意思，它不该反过来改变这张表本身是什么；这时基准已经说不出是谁放的，所以 `confirmed_by` / `confirmed_basis` 等确认字段直接留在那一条成员上。`reference` 成员**不出属性**：它带着键，不被键描述，所以它的列既不单独成为 `concepts[].attributes[]` 的一条，也不会进别的成员也带的那条词根的 `sources[]`——这条规则在语料自己折出来的成员与评审写下的成员上是同一条（见 `concepts[].attributes[]`） |
| `concepts[].attributes[]` | `stem` + `type` + `comment` + `sources[]`（`table` / `column`） | K1：概念是什么做的——成员表的列按词根折成一条，`type` 与 `comment` 取第一个说得出话的来源，`sources[]` 列出每一条来自哪张表的哪一列。**只读角色不是 `reference` 的成员**（K4d）：`reference` 只是带着这个概念的键，不被这个键描述，所以它的列一条也不进来。这份清单每次都从 `tables[]` **当前**的角色重新读一遍，而不是把每次改动并进来——评审用 `add_tables` / `new_concepts` 加的 `reference` 成员照样不出属性，而一次合并把某张表从 `reference` 升成更强的角色之后，它的列从那一刻起就算数 |
