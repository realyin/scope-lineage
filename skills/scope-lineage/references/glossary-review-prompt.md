...
2. 每行的**「候选来源」列**：`comment_enum` / `comment_mention` / `case_label` /
   `case_label(桶 N)` / `same_name_confirmed` / `—`。**只有 `comment_enum`、
   一对一的 `case_label`、`same_name_confirmed` 是下面三类可自答的证据**；
   `comment_mention` 与 `case_label(桶 N)` 不是（见「不算证据的两种」），
   它们和 `—` 一样要去问人。
...
| 2 | 语料里有 CASE **一对一**地把这个取值映射到一个标签（`WHEN status = 'A' THEN '有效'`；「候选来源」为 `case_label`，不带「桶」） | 该标签（`有效`） | `<任务/规则 id> 的 CASE 把该取值标为 <标签>` |
...
- `- 回写目标：` 逐字照抄模板里的键（`<表.列>=<去引号取值>`）。一条问题问几个取值就写几行，
  **一行一个键**：`值域:<表.列>=*` 这种通配写法**不是键**，`--overrides` 认不出来，
  会整条进 `unmatched`。问一整列的时候把那一列的键一个不漏地列出来。
...
| 9 | 自答之前按列过了一遍，两套标签体系并存的列整列没有逐值自答 | |
| 10 | `comment_mention` 与 `case_label(桶 N)` 一条都没当成证据自答过 | |
