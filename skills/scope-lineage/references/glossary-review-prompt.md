...
2. 每行的**「候选来源」列**：`comment_enum` / `comment_mention` / `case_label` /
   `case_label(桶 N)` / `case_label(单值分支)` / `case_label(体系 k/N)` / `code_alias` /
   `same_name_confirmed` / `—`。**只有 `comment_enum`、
   一对一的 `case_label`、`same_name_confirmed` 是下面三类可自答的证据**；
   其余都不是（见「不算证据的四种」），它们和 `—` 一样要去问人。
...
| 2 | 语料里有 CASE **一对一**地把这个取值映射到一个标签（`WHEN status = 'A' THEN '有效'`；「候选来源」正好是 `case_label`——不带「桶」「单值分支」「体系」，也不是 `code_alias`） | 该标签（`有效`） | `<任务/规则 id> 的 CASE 把该取值标为 <标签>` |
...
| `case_label(单值分支)`（Q1） | 标签虽然只对着这一个取值，但**产出它的那条 CASE 把同一列的别的取值分了桶**——它在做分类，不是在做翻译，这一支只是碰巧只装了一个码 | 和 `case_label(桶 N)` 一样处理：整条 CASE 是一套分类口径，问人的时候连着同一套里的其余取值一起问 |
...
| 10 | `comment_mention`、`case_label(桶 N)`、`case_label(单值分支)`、`code_alias` 一条都没当成证据自答过 | |
