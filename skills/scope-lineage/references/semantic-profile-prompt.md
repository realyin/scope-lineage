...
4. 风险必须优先来自 `diagnostics.json`，不能只根据 `semantic.json.confidence` 猜；两者不一致时
   在附录写「存在交叉校验不一致」并分别列出两个来源的值。数警告时取
...
- **可信度**：`事实` / `推断` / `待确认` 三档之一，一个字段一档（取该行最弱的一档）。写 `待确认`
  的字段必须在第三件里有对应 `Q<n>`。该行的事实里含**已确认回写**（`target_comment_source =
  "patch"`，或取值含义 `meaning.status = "confirmed"`）时写 `事实（已确认）`，并在附录 A2 的
  「已确认项」里列一行。
...
给业务方逐条答的问题，**≤ 15 条**，同类合并（同一个 code 列的多个值合成一条）。每条固定六行，
最后一行是留给业务方填的空位：
...
业务方应能 几分钟答完——问题要能用一句话回答，不要把「请核对整张表」写成一条。
...
——它是给业务方的一页纸，字段多不是把它写长的理由；变大的是字段字典与附录。
...
**A2a 已确认项**（有则必须写）：上一轮答完、这一轮已经落进骨架的项，一行一条，不再作为问题出现。
...
| 12. 待确认清单 ≤ 15 条、六行格式齐全（含 `- 答案：（待填）`）、前 5 条标了「优先」 | | |
