...
任务实例本来就对应某一天，分区日期写死是设计而不是缺陷。要提它就写进附录 A3。

**取数日**取 `task.instance_dates[]`（本实例的日期过滤钉住的那一天或几天，`[]` 表示由变量替换）。
语义卡「口径要点」里写成一句事实：「取数日 20260814」；有两天时写「主表取 20260814，另一侧取前 1 日」，
...
「取数日 20260814」，两天时写「取数日 20260814；<另一侧> 取前 1 日」（差几天照
...
- **可信度**：`事实` / `推断` / `待确认` 三档之一，一个字段一档（取该行最弱的一档）。写 `待确认`
  ...
  在附录 A2b 的待填取值清单里有一行即可。该行的事实里含**已确认回写**（`target_comment_source =
...
| 2 | 注释与推导链语义冲突 | 注释称近 30 天而过滤是 `>= 7`；注释称已支付而聚合不带支付条件；注释称最新状态而窗口只做排名 | 一处冲突一条，最多 2 条，多的进 A2b |
...
| 旧问题 | 现在怎么办 |
| --- | --- |
| 硬编码日期是否由调度平台替换 | 不问。任务实例对应某一天，写死是设计；日期写在「口径要点」的取数日一句里 |
| code / 枚举值 / 常量的业务含义 | 不问。正文写固定一句：「取值含义请填 `glossary.overrides.template.md`，见附录 A2b 的待填取值清单」 |
| 候选键在业务上是否真的唯一 | 只在该键参与了 `fan_out_risks[].status = risk` 的关联时问，且并进第 3 类那一条 |
| 连接键是否真实外键 | 同上 |
| 「这样是否符合业务期望」「请核对整张表」类 | 不问。它不是一句话能答的问题 |

**超出 5 条的候选进附录 A2b「备查项」**，一行一条，不用六行格式，也不计入清单。
...
`[待确认]`，取值本身进 A2b 的待填取值清单，由 `glossary.overrides.template.md` 统一收答案。
...
清单常见 0–2 条（四类都不成立时写「本轮无需确认」），附录的自检表只保留下表前 8 项。章节一件都不能少，压缩的是篇幅不是覆盖面；
...
`待业务确认` 不等于要生成 `Q`**，第三件只收那四类，其余在 A2b 里备查。
