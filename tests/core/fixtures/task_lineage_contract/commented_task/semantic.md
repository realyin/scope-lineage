# 任务语义描述：golden_commented_task

共 1 条写入语句；最终产出表：`mart.channel_summary`（来自 `final_table_states`，已排除会话内关系与目录写入）。
...
- 任务元信息：项目 demo_project；负责人 demo_owner；调度周期 DAY；调度表达式 0 20 3 * * ?；期望日期 2026-09-18；任务描述 每日渠道汇总（合成示例）（元数据事实；证据 task_meta）
...
| --- | --- | --- | --- | --- | --- |
| `dim.channel` | 注释未知 | 聚合来源（aggregate_source） | 1 / 2 | 完整 | `ROOT` |
| `ods.channel_event` | 注释未知 | 聚合来源（aggregate_source） | 4 / 4 | 完整 | `ROOT` |
...
| # | 字段 | 一句话语义 | 结构角色 | 口径 | 追溯 |
| --- | --- | --- | --- | --- | --- |
| 1 | `mart.channel_summary.customer_id` | （注释未知）：直接取自 ods.channel_event.customer_id（注释未知）；注释：客户号 | attribute | — | ✓ |
| 2 | `mart.channel_summary.channel_name` | （注释未知）：channel_code = 'A' → 'ONLINE'；否则 'OFFLINE'；来源 ods.channel_event.channel_code（注释未知）；注释：渠道编码（合成值域） | conditional_label | — | ✓ |
| 3 | `mart.channel_summary.total_amount` | （注释未知）：按 customer_id、CASE WHEN channel_code = 'A' THEN 'ONLINE' ELSE 'OFFLINE' END 聚合：SUM(amount)；来源 ods.channel_event.amount（注释未知）；注释：金额合计（元） | measure | SUM(amount)；未在聚合路径上发现日期过滤 | ✓ |

### 字段 mart.channel_summary.customer_id

- 语义：（注释未知）：直接取自 ods.channel_event.customer_id（注释未知）；注释：客户号
- 注释：客户号（SQL注释）
- 目标注释：注释未知（元数据事实）
- 类型：未知（元数据事实）
...
- 语义：（注释未知）：channel_code = 'A' → 'ONLINE'；否则 'OFFLINE'；来源 ods.channel_event.channel_code（注释未知）；注释：渠道编码（合成值域）
- 注释：渠道编码（合成值域）（SQL注释）
...
- 语义：（注释未知）：按 customer_id、CASE WHEN channel_code = 'A' THEN 'ONLINE' ELSE 'OFFLINE' END 聚合：SUM(amount)；来源 ods.channel_event.amount（注释未知）；注释：金额合计（元）
- 口径：
...
- ⚠ table_comment_missing：缺少表注释的表（3 张）：dim.channel、ods.channel_event、mart.channel_summary（元数据事实）
