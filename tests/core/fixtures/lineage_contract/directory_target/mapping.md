...
- 步骤 1/1：`ods.users.user_id` → `directory:/warehouse/export/daily.user_id`；direct_projection；表达式：`` `user_id` ``
...
- 加工路径：1 步；direct_projection
- 步骤 1/1：`ods.users.user_name` → `directory:/warehouse/export/daily.user_name`；direct_projection；表达式：`` `user_name` ``
