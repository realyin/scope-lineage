...
lineage_digest: "c067a3d853c88d59"
---
...
- 目标表元数据来源：无（元数据事实）
- 结构摘要：ROOT 直接读取 ods.users；输出 2 列。（结构推断；证据 scope_profile）
...
- ⚠ table_comment_missing：缺少表注释的表（2 张）：ods.users、directory:/warehouse/export/daily（元数据事实）
