...
- 目标表：`mart.flags`（表注释：注释未知）（元数据事实）
- 语句类型：INSERT；无分区（SQL事实）
- 目标表元数据来源：无（元数据事实）
- 结构摘要：ROOT 直接读取 ods.users；CASE WHEN 派生 1 个字段；输出 1 列。（结构推断；证据 scope_profile）

共 1 张输入表（角色为结构推断，其余为 SQL/元数据事实）：

| 输入表 | 表注释 | 角色 | 使用列 / 全宽 | 元数据 | 读取 scope |
| --- | --- | --- | --- | --- | --- |
| `ods.users` | 注释未知 | 主表（driving） | 1 / 1 | 完整 | `ROOT` |
...
### 阶段 1：ROOT（`ROOT`，角色 transform）

- 直接输入：`ods.users`（SQL事实）
- 直接读取物理表：`ods.users`（SQL事实）
- 动作：
  - 条件派生：派生 flagged：name = 'x；y → z|w' → 'a\nb'；否则 name（SQL事实；证据 logic:ROOT:case_when:001）
- 输出：1 列（`flagged`）（SQL事实）

## 4. 规则清单

共 1 条规则（CASE 分支 1 条）；条件为 SQL 原文，字段注释为元数据事实。

| 规则 | 类型 | 阶段 | 条件 | 涉及字段（注释） | SQL注释 | 分区过滤 | 证据 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| rule:001 | CASE 分支（case_branch） | `ROOT` | `name = 'x；y → z\|w' → 'a\nb'；否则 name` | `ods.users.name`（注释未知） | — | — | `logic:ROOT:case_when:001` |
...
| # | 字段 | 一句话语义 | 结构角色 | 口径 | 追溯 |
| --- | --- | --- | --- | --- | --- |
| 1 | `mart.flags.flagged` | （注释未知）：name = 'x；y → z\|w' → 'a\nb'；否则 name；来源 ods.users.name（注释未知） | derived | — | ✓ |

### 字段 mart.flags.flagged

- 语义：（注释未知）：name = 'x；y → z|w' → 'a\nb'；否则 name；来源 ods.users.name（注释未知）
- 取值：'a\nb'（待确认）（SQL事实；证据 rule:001）
- 目标注释：注释未知（元数据事实）
- 类型：未知（元数据事实）
- 来源：`ods.users.name`（注释未知）（SQL事实）
- 结构角色：derived（末步变换 CONDITIONAL）（结构推断；证据 mc:001）
- 加工步骤：
  - 步骤 1/1 @ `ROOT`（case_when）：name = 'x；y → z|w' → 'a\nb'；否则 name；粒度=preserved（SQL事实）
- 最终表达式：`` CASE WHEN `users`.`name` = 'x；y → z|w' THEN 'a\nb' ELSE `users`.`name` END ``（SQL事实）
...
- ⚠ table_comment_missing：缺少表注释的表（2 张）：ods.users、mart.flags（元数据事实）
