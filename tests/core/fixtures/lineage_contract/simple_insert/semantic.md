...
lineage_digest: "ee7246d6cfdfe5e6"
---

# 任务语义描述 mart.user_names
...
| --- | --- | --- | --- | --- | --- |
| `ods.users` | 注释未知 | 主表（driving） | 3 / 3 | 完整 | `ROOT` |
...
- 动作：
  - 过滤：只保留 is_active = 1 的记录（SQL事实；证据 logic:ROOT:filter:001）
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| rule:001 | 过滤（filter） | `ROOT` | `` `s`.`is_active` = 1 `` | `ods.users.is_active`（注释未知） | — | 否 | `logic:ROOT:filter:001` |
...
| # | 字段 | 一句话语义 | 结构角色 | 口径 | 追溯 |
| --- | --- | --- | --- | --- | --- |
| 1 | `mart.user_names.user_id` | （注释未知）：直接取自 ods.users.user_id（注释未知） | attribute | — | ✓ |
| 2 | `mart.user_names.user_name` | （注释未知）：直接取自 ods.users.user_name（注释未知） | attribute | — | ✓ |
...
- ⚠ table_comment_missing：缺少表注释的表（2 张）：ods.users、mart.user_names（元数据事实）
