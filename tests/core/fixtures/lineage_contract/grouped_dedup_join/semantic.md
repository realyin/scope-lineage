...
- 结构摘要：行来源 ods.events_a（经 union:events_norm:b01 → union:events_norm → cte:events_norm → cte:agg → cte:joined → cte:ranked）、ods.events_b（经 union:events_norm:b02 → union:events_norm → cte:events_norm → cte:agg → cte:joined → cte:ranked）；关联 1 个上游（1 个去重）；输出 6 列。（结构推断；证据 scope_profile）
