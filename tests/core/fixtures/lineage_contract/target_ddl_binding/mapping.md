...
- 目标绑定：applied；方法=ddl_position；投影 2 → 目标列 2；纠正 2
...
- 步骤 1/1：`ods.account_snapshot.account_key` → `dwd.account_daily.account_id`；direct_projection；表达式：`` `account_key` ``
...
- 加工路径：1 步；direct_projection
- 步骤 1/1：`ods.account_snapshot.balance_value` → `dwd.account_daily.balance`；direct_projection；表达式：`` `balance_value` ``
...
- 输入：ods.account_snapshot；物理上游：ods.account_snapshot
