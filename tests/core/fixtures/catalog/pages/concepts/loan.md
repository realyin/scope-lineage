...
实体 · [贷款](../index.md) · 本页 12% 草拟、已确认的条目标 ✓

## 一页纸概览

**是什么**：One amount lent to a borrower, repaid over one or more instalments.

**怎么认出来**：

- 借据号：一开始就有，全局唯一（主标识） ✓

**状态**：正常 → 逾期 —豁免→ 结清（另：逾期 —还款→ 正常；正常 —还款→ 结清）

**数据在哪**：

- `demo_dwd.dwd_lending_loan_df`（Loan snapshot） ✓
- `demo_dws.dws_lending_loan_summary_1d` 已废弃，改用 `demo_dws.dws_lending_loan_summary_v2_1d`
- 另有 3 张表带本概念的标识，分布在 2 个域（见附录 A3）

**关联的**：is owed by 借款人 ✓、分期借据 is a kind of 借据、renews 借据
