...
That can turn one missed relation into many downstream gaps.
"""
...
CACHE_THEN_READ_SQL = """
cache lazy table session_cached_relation OPTIONS ('storageLevel' 'DISK_ONLY') as select id, v from ods.s;
INSERT INTO mart.t SELECT t.id, t.v FROM session_cached_relation t
"""
...
def test_a_downstream_read_of_a_cached_relation_resolves_to_its_real_source() -> None:
    ...
    assert "session_cached_relation" not in coverage["missing_tables"]
    ...
    assert {
        (item["column"], source["table"], source["column"])
        for item in result.end_to_end_lineage
        if item["table"] == "session_cached_relation"
        for source in item["value_sources"]
        if source.get("source_kind") == "physical_field"
    ...
    assert [
        ...
        if node["table"] == "session_cached_relation"
    ] == [("session_cached_relation", "stmt:001")]
...
def test_a_cached_relation_whose_projection_stayed_a_wildcard_is_not_registered() -> None:
    ...
    results = parse_all_scope_lineage(
        """
        cache lazy table session_cached_relation as select * from ods.undocumented;
        INSERT INTO mart.t SELECT id, v FROM session_cached_relation
        """,
        "cache_star",
        schema=SCHEMA,
    )
