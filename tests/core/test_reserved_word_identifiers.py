...
Spanish-language features where `not` sits between `notado` and `noticia`. Spark accepts them
quoted; the author did not quote them, and sqlglot's parser stops at the first one. The statement
falls to the lenient parse, its projection list is discarded, and 1298 of 1299 output columns come
back with no source at all. Two identifiers out of 2943 projections cost the entire statement
(KEYWORD-IDENT-001).
...
Clause keywords are never quoted, and that guard is not theoretical: one real task's malformed
