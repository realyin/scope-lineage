...
- **`docs/` is two trees, `docs/zh-CN/` and `docs/en/`, and they are one deliverable.**
  A documentation change lands in both languages in the same commit; the guard
  (`python tests/architecture/docs_parity.py`, also a test) compares file sets, heading
  skeletons, table shapes, code blocks — JSON/YAML/CSV examples byte-for-byte — and link
  targets, and fails on a tree that drifted. `examples/README.md` and
  `examples/README.zh-CN.md` are checked as the same kind of pair.
- When you remove or rename public API, a CLI flag/default, or an output layout,
  grep ALL of `docs/` (both languages), both READMEs, and `skills/` for the old name and
  every code example that uses it, in the same change. The 0.2.0 release updated the
  READMEs but left six docs describing the removed contract as the default — including a
  Python example that raised ImportError if copied.
